...
// for authentication
// String  uid = '4335435';
// String  displayName = 'NP';
// String  email = 'np@yopmail.com';
// String  photoURL = 'fsdf';
//
String uid = '123343';
String displayName = 'SP';
String email = 'sp@yopmail.com';
String photoURL = 'fsdf';
...
class APIs {
  ...
  static Future<bool> addChatUser(String email) async {
    ...
    if (data.docs.isNotEmpty && data.docs.first.id != uid) {
      ...
      firestore.collection('users').doc(uid).collection('my_users').doc(data.docs.first.id).set({});
  ...
  static Future<void> getSelfInfo() async {
    await firestore.collection('users').doc(uid).get().then((user) async {
  ...
  static Future<void> createUser() async {
    ...
    return await firestore.collection('users').doc(uid).set(chatUser.toJson());
  }

  // for getting id's of known users from firestore database
  static Stream<QuerySnapshot<Map<String, dynamic>>> getMyUsersId() {
    return firestore.collection('users').doc(uid).collection('my_users').snapshots();
  }
  ...
  static Future<void> sendFirstMessage(ChatUser chatUser, String msg, Type type) async {
    await firestore.collection('users').doc(chatUser.id).collection('my_users').doc(uid).set({}).then((value) => sendMessage(chatUser, msg, type));
  }
  ...
  static Future<void> updateProfilePicture(File file) async {
    ...
    await firestore.collection('users').doc(uid).update({'image': me.image});
  }
  ...
  static Future<void> sendMessage(ChatUser chatUser, String msg, Type type) async {
    ...
    //message to send
    final Message message = Message(toId: chatUser.id, msg: msg, read: '', type: type, fromId: uid, sent: time);
